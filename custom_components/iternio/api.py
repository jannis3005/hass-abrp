"""API for A Better Route Planner bound to Home Assistant OAuth."""
...
class ConfigEntryAuth(my_pypi_package.AbstractAuth):
    """Provide A Better Route Planner authentication tied to an OAuth2 based config entry."""
    ...
    def __init__(
        ...
        """Initialize A Better Route Planner Auth."""
        self.hass = hass
        self.session = oauth_session
        super().__init__(self.session.token)

    def refresh_tokens(self) -> str:
        """Refresh and return new A Better Route Planner tokens using Home Assistant OAuth2 session."""
